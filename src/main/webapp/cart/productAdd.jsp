...
<script >
	...
	$(function() {
		$("a[lang='mod']").click(function(){		
		 ...
		 $("#save").click(function(){
			 var state=$("#save").text();
			 if(state=='수정')$("form").attr("action","/web/adminModify.do");
			 $("form").submit();		 
		 });	
	});
	  	/* $("a[lang='mod']").click(function(){
			...
			$.ajax({
				...
				success:function(data){	
					...
					$("productPrice").val(data.productPrice);
					$("productCategory").val(data.productCategory);
					$("#pk").val(this.id); //pk가 hidden으로 넘어감
					$("#save").text("수정");
				},
				error:function(){		
					alert('error');
								...
								<tbody>
								 <c:forEach var="i" items="${adminList}" varStatus="cnt"> 
									<tr>
										...
										<td>
										  <span class="buttonFuc"><a href="#" id="${i.pk}" lang="mod" >수정</a></span>  
										<span class="buttonFuc"><a  href="deleteAdminProduct.do?pk=${i.pk}">삭제</a></span>
										</td>
									</tr>	
										</c:forEach>				
